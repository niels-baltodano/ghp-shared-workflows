...
branch_flutter_read_pubspec_version() {
  ...
  [[ -f "${pubspec_file}" ]]       || { log_error "pubspec.yaml not found at '${pubspec_file}'"; return 1; }
  command -v yq >/dev/null 2>&1    || { log_error "'yq' is not installed or not in PATH"; return 1; }

  local version
  version="$(yq -r '.version' "${pubspec_file}")"

  [[ -n "${version}" && "${version}" != "null" ]] \
    || { log_error "Could not read '.version' from pubspec.yaml"; return 1; }

  [[ "${version}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\+[0-9]+$ ]] \
    || { log_error "pubspec.yaml version '${version}' does not match expected format X.Y.Z+BUILD"; return 1; }
...
branch_validate_release() {
  ...
  if [[ "${is_flutter}" == "1" ]]; then
    ...
    [[ "${pubspec_version}" == "${version_without_v}" ]] \
      || { log_error "[${context}] Branch version '${version_without_v}' does not match pubspec.yaml '${pubspec_version}'"; return 1; }
