...
# @example
#   _ctx_strip_ref_prefix "refs/heads/release/v1.2.3"
#   # Output: "release/v1.2.3"
#
#   _ctx_strip_ref_prefix "main"
...
# @example
#   declare -A CI_CTX
#   ctx_build CI_CTX "pull_request" "refs/heads/release/v1.2.3" "main" \
#     "release/v1.2.3" "container" "abc1234def" "owner/my-repo"
#   # CI_CTX[event]="pull_request"
#   # CI_CTX[head]="release/v1.2.3"
#   # CI_CTX[base]="main"
#   # CI_CTX[ref_name]="release/v1.2.3"
#   # CI_CTX[target]="container"
#   # CI_CTX[sha]="abc1234def"
#   # CI_CTX[should_run]="false"   (set to true by policy_evaluate on success)
# ------------------------------------------------------------
ctx_build() {
	local -n _ctx_b="$1"
	local event="$2"
	...
	# On push/dispatch events GITHUB_HEAD_REF is empty; fall back to ref_name
	# so CTX_F_HEAD always holds the semantically active branch name.
...
#   ctx_is_pr       "push"              && echo "is PR"       # prints nothing
...
#   ctx_is_container_project "/workspace" && echo "container project"  # prints it
#
#   # /workspace/Dockerfile does not exist:
#   ctx_is_container_project "/workspace" || echo "not a container project"
# ------------------------------------------------------------
...
# @example
#   # /workspace/pubspec.yaml exists:
#   ctx_is_flutter_project "/workspace" && echo "flutter project"  # prints it
#
#   # /workspace/pubspec.yaml does not exist:
#   ctx_is_flutter_project "/workspace" || echo "not a flutter project"
# ------------------------------------------------------------
...
# @example
#   ctx_validate_target "container" && echo "valid"    # prints "valid"
#   ctx_validate_target "flutter"   && echo "valid"    # prints "valid"
#   ctx_validate_target "java"      || echo "invalid"  # prints "invalid"
# ------------------------------------------------------------
ctx_validate_target() {
	local target="$1"
	case "${target}" in
