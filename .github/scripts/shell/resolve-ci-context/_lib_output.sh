...
# @example
#   # After ctx_build + policy_evaluate on a PR from release/v1.2.3 → main:
#   output_export "CI_CTX" "/workspace" "${GITHUB_OUTPUT}"
#   # Appends to GITHUB_OUTPUT:
#   #   should_run=true
#   #   target_action=container
#   #   release_version=v1.2.3
#   #   release_version_number=1.2.3
#   #   build_number=
#   #   is_pr=true
#   #   is_push=false
#   #   is_dispatch=false
...
output_export() {
	...
	# ── Derive event-type flags from the event string ─────────────
	# Exactly one of these will be true per run.
	...
	# ── Derive project-type flags from workspace file presence ────
	# A repo can have both Dockerfile and pubspec.yaml; both flags may be true.
	...
	# ── Resolve the semantically active branch for this event ─────
	# PR  → head branch (the source of the changes)
	# Push/Dispatch → ref_name (the branch that triggered the run)
	local active_branch
...
# @param $2     workspace - Absolute path to GITHUB_WORKSPACE.
# @example
#   output_summary "CI_CTX" "/workspace"
#   # Prints to stderr:
#   # ════════════════════════════════════════
#   #   Policy Gate Summary
#   # ════════════════════════════════════════
